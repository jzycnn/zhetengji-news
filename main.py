...
def generate_html():
    ...
    for feed in feeds:
        try:
            ...
            # 容错：如果解析失败，跳过该源
            if hasattr(f, 'bozo_exception') and f.bozo_exception:
                print(f"  - 解析警告: {f.bozo_exception}")

            for entry in f.entries[:8]: # 每个源取前8条
                # 1. 尝试获取内容 HTML 以提取图片
                content_html = ""
                ...
                if hasattr(entry, 'published'):
                    pub_time = entry.published[:16] # 截取年月日时分
    ...
    <head>
        ...
        <meta name="description" content="折藤记 - 每日自动更新的 IT 科技新闻聚合站">
        <title>折藤记 - 科技早报</title>
        <style>
            :root {{
                --primary: #d32f2f;
                --bg: #f5f7fa;
            ...
            .no-img {{ display: flex; align-items: center; justify-content: center; background: #eee; color: #999; font-weight: bold; font-size: 1.2rem; }}
            ...
            .card-title {{ margin: 0; font-size: 1rem; line-height: 1.5; color: #333; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; }}
            
            footer {{ text-align: center; padding: 3rem 0; color: #999; font-size: 0.85rem; }}
    ...
    <body>
        <header>
            <div class="header-content">
                <div class="brand">折藤记</div>
                <div class="time">更新: {update_time}</div>
            </div>
        </header>
        <main>{cards_html}</main>
        <footer>
            <p>聚合全网科技热点 | 备案名：折藤记</p>
            <p>&copy; {datetime.datetime.now().year} Powered by EdgeOne Pages</p>
        </footer>
    </body>
    </html>
    """
